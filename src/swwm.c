...
struct swwm_server {
	...
	struct swwm_toplevel *grabbed_toplevel;
...
static void focus_toplevel(struct swwm_toplevel *toplevel) {
...
static bool handle_keybinding(struct swwm_server *server, xkb_keysym_t sym) {
	/*
	...
	case XKB_KEY_F1:
		...
		struct swwm_toplevel *next_toplevel =
...
static void server_new_input(struct wl_listener *listener, void *data) {
	 ...
	 * communiciated to the client. In swwm we always have a cursor, even if
...
static void seat_request_set_selection(struct wl_listener *listener, void *data) {
	 ...
	 * ignore such requests if they so choose, but in swwm we always honor
	 */
	struct swwm_server *server = wl_container_of(
...
static struct swwm_toplevel *desktop_toplevel_at(
		struct swwm_server *server, double lx, double ly,
		struct wlr_surface **surface, double *sx, double *sy) {
	/* This returns the topmost node in the scene at the given layout coords.
	 * We only care about surface nodes as we are specifically looking for a
	 * surface in the surface tree of a swwm_toplevel. */
	...
	*surface = scene_surface->surface;
	/* Find the node corresponding to the swwm_toplevel at the root of this
...
static void reset_cursor_mode(struct swwm_server *server) {
	...
	server->cursor_mode = swwm_CURSOR_PASSTHROUGH;
	server->grabbed_toplevel = NULL;
}

static void process_cursor_move(struct swwm_server *server) {
...
static void process_cursor_resize(struct swwm_server *server) {
	/*
...
static void process_cursor_motion(struct swwm_server *server, uint32_t time) {
	/* If the mode is non-passthrough, delegate to those functions. */
	if (server->cursor_mode == swwm_CURSOR_MOVE) {
		process_cursor_move(server);
		return;
	} else if (server->cursor_mode == swwm_CURSOR_RESIZE) {
	...
	struct swwm_toplevel *toplevel = desktop_toplevel_at(server,
...
static void xdg_toplevel_commit(struct wl_listener *listener, void *data) {
	...
	if (toplevel->xdg_toplevel->base->initial_commit) {
		 ...
		 * reply with a configure so the client can map the surface. swwm
...
static void xdg_toplevel_request_move(
	...
	begin_interactive(toplevel, swwm_CURSOR_MOVE, 0);
}
...
static void xdg_toplevel_request_resize(
	...
	begin_interactive(toplevel, swwm_CURSOR_RESIZE, event->edges);
}
...
static void xdg_toplevel_request_maximize(
	 ...
	 * decorations. swwm doesn't support maximization, but to conform to
...
static void server_new_xdg_toplevel(struct wl_listener *listener, void *data) {
	...
	/* Allocate a swwm_toplevel for this surface */
...
static void xdg_popup_commit(struct wl_listener *listener, void *data) {
	...
	if (popup->xdg_popup->base->initial_commit) {
		 ...
		 * swwm sends an empty configure. A more sophisticated compositor
